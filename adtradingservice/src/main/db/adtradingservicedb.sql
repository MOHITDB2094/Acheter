...
CREATE TABLE `post_order` (
  ...
  `order_created_dt` varchar(45) DEFAULT NULL,
...
CREATE TABLE `category` (
  ...
  `category_image_no` int DEFAULT NULL,
  PRIMARY KEY (`category_id`),
  UNIQUE KEY `category_nm_UNIQUE` (`category_nm`),
  UNIQUE KEY `display_sequence_no_UNIQUE` (`display_sequence_no`),
  KEY `parent_category_fk_idx` (`category_image_no`),
  CONSTRAINT `parent_category_fk` FOREIGN KEY (`category_image_no`) REFERENCES `images` (`image_id`) ON DELETE RESTRICT ON UPDATE RESTRICT
...
CREATE TABLE `images` (
  ...
  `last_modified_dt` datetime DEFAULT NULL,
  `last_modified_by` varchar(50) DEFAULT NULL,
  `content_type` varchar(70) DEFAULT NULL,
...
CREATE TABLE `product_exchange_request` (
  ...
  `last_modified_dt` datetime DEFAULT NULL,
  `last_modified_by` varchar(50) DEFAULT NULL,
  `exchage_customer_id` int NOT NULL,
...
-- Dump completed on 2021-07-14  9:13:19
